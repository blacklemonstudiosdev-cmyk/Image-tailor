...
// If Pollinations' image-edit endpoint differs, change this one line (or set the
// POLLINATIONS_EDIT_URL environment variable in Vercel — no code change needed).
...
export default async function handler(req, res){
  try {
    ...
    if (image){
      // ---- IMAGE EDIT ----
      // Send the source image as a base64 data URL inside a JSON body (not a file).
      // If the contract differs, change EDIT_ENDPOINT / field names below; it fails
      // gracefully (502) and the error shows the upstream HTTP status.
      const b64raw = (s) => { const mm = /^data:[^;]+;base64,(.*)$/.exec(s); return mm ? mm[1] : s; };
      const r = await fetch(EDIT_ENDPOINT, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ prompt: prompt, image: image, model: 'flux', size: width + 'x' + height })
      });
      if (!r.ok){
        res.status(502).json({ error: 'upstream', reason: 'Image editing endpoint returned HTTP ' + r.status + ' (check POLLINATIONS_EDIT_URL / api/generate.js).' });
        return;
      }
      ...
      if (ct.indexOf('application/json') >= 0){
        ...
        if (item && item.b64_json){
          ...
          if (ir.ok){ outBuf = Buffer.from(await ir.arrayBuffer()); outCt = ir.headers.get('content-type') || 'image/png'; }
        } else if (j && j.images && j.images[0]){
          outBuf = Buffer.from(b64raw(String(j.images[0])), 'base64');
        }
        if (!outBuf){ res.status(502).json({ error: 'upstream', reason: 'The editor returned an unexpected response shape.' }); return; }
